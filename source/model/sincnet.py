...
class SincLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        # Add Safe Padding
        self.filter_low_cutoff = tf.math.abs(self.filter_low_freq) + self.min_hz_freq / self.sample_rate
        self.filter_high_cutoff = self.filter_low_cutoff + tf.math.abs(self.filter_bandwidth) + self.min_hz_bandwidth / self.sample_rate
        ...
        window = 0.54 - 0.46 + tf.math.cos(2 * math.pi * n / self.filter_dim)
        self.window = tf.constant(tf.dtypes.cast(tf.reshape(window, [1, -1]), dtype=tf.float32), dtype=tf.float32)
        ...
        _tmp_list = [tf.reverse(band_pass, axis=[1]), tf.zeros([self.n_filter, 1], tf.float32), band_pass]
        self.band_pass = tf.concat(_tmp_list, 1)
